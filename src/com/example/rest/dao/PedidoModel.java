...
public class PedidoModel {
	...
	public int inserta(Pedido pedido) {
		System.out.println("---> En MySqlPedido-> inserta");
		...
		try {
			...
			System.out.println(pedido.getUbigeo().getIdUbigeo()+"");
			// se crea el sql de la cabecera
			String sql1 = "insert into pedido values(null,curtime(),?,?,?,?,1111,?)";
			pstm1 = conn.prepareStatement(sql1);
			pstm1.setDate(1,pedido.getFechaEntrega());
			pstm1.setString(2, pedido.getLugarEntrega());
			pstm1.setString(3, pedido.getEstado());
			pstm1.setInt(4, pedido.getCliente().getIdCliente());
			pstm1.setInt(5,pedido.getUbigeo().getIdUbigeo());
			pstm1.setInt(6, pedido.getUsuario().getIdUsuario());
			pstm1.executeUpdate();
